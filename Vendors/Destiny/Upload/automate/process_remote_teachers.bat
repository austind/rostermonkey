...
IF "%SOURCEFILE%"=="" SET SOURCEFILE=Teachers.csv

REM OPTIONAL VARIABLES USING DEFAULT VAULES
IF "%SOURCEFULLFILE%"=="" SET SOURCEFULLFILE=%SOURCEDIR%\%SOURCEFILE%
IF "%LOGFILE%"=="" SET LOGFILE=.\Logs\Staff.log
IF "%FINAL%"=="" SET FINAL=.\patdata\Teachers.xml
IF "%PATRON_PROP%"=="" SET PATRON_PROP=.\config\map_teachers.properties
IF "%UPDATE%"=="" SET UPDATE=.\config\update_teachers.properties
IF "%SORTED%"=="" SET SORTED=.\patdata\sorted_teachers.csv
IF "%NOCHGXML%"=="" SET NOCHGXML=.\patdata\destiny_staff_nochange.xml
IF "%NOCHGUPD%"=="" SET NOCHGUPD=update_no_change.properties
IF "%RAWFILE%"=="" SET RAWFILE=.\patdata\Teachers.csv

REM ********************************************************************************
REM * ONLY USE IF MULITPLE COPIES OF THE CSV FILE CONTAIN THE SAME OUTPUT
REM * (Uncomment the below line if needed)
REM ********************************************************************************
REM copy /Y %SOURCEDIR%\*.csv %SOURCEDIR%\Teachers.csv
