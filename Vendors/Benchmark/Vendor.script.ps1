...
# These headers can't be set directly in PS versions less than 5
# See Vendor.script.v5.ps1 for the updated version
$Headers = @{
    #'Accept'                    = '*/*'
...
$Request = @{
    # Method is always POST when sending data
    Method = 'POST'
    ...
    # In PS3.x, these are parameters, not headers.
    # In PS5.x, these are set directly in the header dictionary above
    UserAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.96 Safari/537.36'
    ContentType = 'multipart/form-data; boundary=----WebKitFormBoundarynydMOBVUvB5ysQvF'
}
...
"@
    ...
    # I really doubt anyone on their end is auditing headers for requests to /root/logout, but best to be safe
    $Request['Method'] = 'GET'
